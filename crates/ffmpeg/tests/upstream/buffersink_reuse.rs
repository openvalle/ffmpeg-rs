// Ported from ffmpeg-next 9.0.0; archive and license are pinned in NOTICE.
use super::ffmpeg;

use self::ffmpeg::{ffi, filter, format, frame};
...
fn buffersink_frame_reuse_drains_all_frames() {
    assert_eq!(valle_ffmpeg::init().unwrap().version.major(), super::MAJOR);
    ffmpeg::init().unwrap();
