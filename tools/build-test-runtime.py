...
def copy_toolchain_dlls(directory, compiler_directory):
    """Make CI DLL fixtures self-contained, including transitive MinGW support DLLs."""
    pending = list(directory.glob("*.dll"))
    seen = set()
    while pending:
        dll = pending.pop()
        ...
        imports = re.findall(r"DLL Name:\s*(\S+)", subprocess.check_output(
            ["objdump", "-p", str(dll)], text=True))
        print(f"{dll.name} imports: {', '.join(imports)}", flush=True)
        for name in imports:
            if name.lower() == "msys-2.0.dll" or name.lower().startswith("cygwin"):
                raise RuntimeError(f"{dll.name} unexpectedly depends on the MSYS shell runtime")
            destination = directory / name
            if destination.is_file():
                pending.append(destination)
                continue
            source = compiler_directory / name
            if source.is_file():
                shutil.copy2(source, destination)
                pending.append(destination)
            elif name.lower().startswith("lib"):
                raise RuntimeError(f"missing MinGW support DLL: {name}")
            # Other imports are Windows system DLLs resolved by the native loader.
...
def main():
    ...
    if args.fingerprint:
        ...
        digest = hashlib.sha256(Path(__file__).read_bytes() + compiler + identity)
        if windows:
            compiler_directory = Path(shutil.which("gcc")).resolve().parent
            for dll in sorted(compiler_directory.glob("*.dll")):
                digest.update(dll.name.encode())
                digest.update(hashlib.sha256(dll.read_bytes()).digest())
    ...
    if windows:
        ...
        if not compiler:
            raise SystemExit("MinGW gcc is missing from PATH")
        copy_toolchain_dlls(prefix / "bin", Path(compiler).resolve().parent)
